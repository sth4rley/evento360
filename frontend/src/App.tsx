...
function ParticipantsPage({ eventId, organizer, onLogout }: { eventId: string; organizer: Organizer; onLogout: () => void }) {
  ...
  return <AdminLayout active="participants" eventId={eventId} onLogout={onLogout} organizer={organizer}>
    ...
    {data ? <><PageTitle title={data.event.name} subtitle={`${dateFormat.format(new Date(data.event.date))} • ${data.event.location}`} actions={<Button className="button-secondary" onClick={() => navigate(`/admin/events/${eventId}`)}><AppIcon name="arrow-left" size={17} /> Voltar</Button>} />
      <div className="metrics-grid"><Metric title="Capacidade" value={data.metrics.capacity} /><Metric title="Inscritos" value={data.metrics.activeRegistrations} /><Metric title="Vagas disponíveis" value={data.metrics.availableSeats} /><Metric title="Comparecimento" value="—" detail="Acesse check-in" /></div>
      <div className="table-card"><header><div><h2>Participantes</h2><p>{data.metrics.activeRegistrations} inscrições ativas</p></div></header>{data.participants.length === 0 ? <div className="empty-state">Ainda não há inscritos neste evento.</div> : <div className="table-scroll"><table><thead><tr><th>Nome</th><th>E-mail</th><th>Telefone</th><th>Status</th><th>Check-in</th></tr></thead><tbody>{data.participants.map((participant) => <tr key={participant.id}><td><strong>{participant.participantName}</strong>{participant.confirmationCode ? <small>{participant.confirmationCode}</small> : null}</td><td>{participant.participantEmail}</td><td>{participant.participantPhone || "—"}</td><td><span className={`badge ${participant.status === "ACTIVE" ? "badge-green" : "badge-gray"}`}>{participant.status === "ACTIVE" ? "Confirmado" : "Cancelado"}</span></td><td><span className={`badge ${participant.checkedInAt ? "badge-green" : "badge-gray"}`}>{participant.checkedInAt ? "Presente" : "Pendente"}</span></td></tr>)}</tbody></table></div>}</div></> : null}
  </AdminLayout>;
}
...
function ParticipantEventsPage({ user, onLogout }: { user: ParticipantAccount; onLogout: () => void }) {
  ...
  return <ParticipantLayout onLogout={onLogout} user={user}>
    ...
    <div className="event-grid">{registrations.map((registration, index) => <article className="event-card participant-event-card" key={registration.id}>
      ...
      <div className="participant-event-badges"><span className={`badge ${registration.status === "ACTIVE" ? "badge-green" : "badge-gray"}`}>{registration.status === "ACTIVE" ? "Inscrição ativa" : "Inscrição cancelada"}</span>{registration.confirmationCode ? <span className="badge badge-blue">{registration.confirmationCode}</span> : null}</div>
      ...
      <div className="button-row"><Button className="button-primary" disabled={!registration.event.available} onClick={() => navigate(`/event/${encodeURIComponent(registration.event.publicId)}`)}>Ver evento</Button>{registration.confirmationCode ? <Button className="button-secondary" onClick={() => navigator.clipboard?.writeText(registration.confirmationCode!)}>Copiar código</Button> : null}{registration.status === "ACTIVE" && registration.cancellationToken ? <Button className="button-danger" onClick={() => navigate(`/registration/cancel/${encodeURIComponent(registration.cancellationToken!)}`)}>Cancelar inscrição</Button> : null}</div>
    </article>)}</div>
  </ParticipantLayout>;
}
...
function EventLookupPage() {
  ...
  return <PublicLayout><section className="lookup-wrap">
    ...
    {registration ? <article className="lookup-result registration-proof-preview"><div><span className={`badge ${registration.status === "ACTIVE" ? "badge-green" : "badge-gray"}`}>{registration.status === "ACTIVE" ? "Inscrição ativa" : "Inscrição cancelada"}</span><h2>{registration.event.name}</h2><p><AppIcon name="user" size={17} /> {registration.participantName}</p><p><AppIcon name="calendar" size={17} /> {dateFormat.format(new Date(registration.event.date))}</p><p><AppIcon name="map-pin" size={17} /> {registration.event.location}</p><p><AppIcon name="ticket" size={17} /> Código: {registration.confirmationCode}</p></div><Button className="button-primary" onClick={() => navigate(`/registration/${encodeURIComponent(registration.confirmationCode)}`)}>Abrir comprovante <AppIcon name="arrow-right" size={17} /></Button></article> : null}
  ...
  return <PublicLayout>{error ? <div className="public-error"><h1>Evento não encontrado</h1><p>{error}</p></div> : !event ? <p className="loading-text public-loading">Carregando evento…</p> : <>
    ...
    <section className="public-content">
      ...
      <aside className="public-info-card public-registration">
        ...
        <p>A inscrição online leva menos de 1 minuto.</p>
        {event.isFull ? <Button className="button-secondary button-block" disabled>Evento lotado</Button> : <Button className="button-primary button-block" onClick={() => navigate(`/event/${event.publicId}/register`)}>Inscrever-se <AppIcon name="arrow-right" size={18} /></Button>}
...
function RegistrationPage({ publicId, participant }: { publicId: string; participant: ParticipantAccount | null }) {
  ...
  useEffect(() => { let mounted = true; apiRequest<{ event: PublicEvent }>(`/api/public/events/${publicId}`).then((result) => mounted && setEvent(result.event)).catch((requestError) => mounted && setError(messageFrom(requestError))); return () => { mounted = false; }; }, [publicId]);
  async function submit(form: FormEvent<HTMLFormElement>) { form.preventDefault(); setLoading(true); setError(null); try { const result = await apiRequest<{ registration: { confirmationCode: string; cancellationToken: string } }>(`/api/public/events/${publicId}/registrations`, { method: "POST", auth: participant ? "participant" : undefined, body: JSON.stringify({ participantName: name, participantEmail: email, participantPhone: phone }) }); navigate(`/registration/${result.registration.confirmationCode}?cancel=${result.registration.cancellationToken}`); } catch (requestError) { setError(messageFrom(requestError)); } finally { setLoading(false); } }
  return <PublicLayout back={() => navigate(`/event/${publicId}`)}><section className="registration-wrap"><form className="form-card" onSubmit={submit}><span className="badge badge-blue">{event?.name || "Inscrição"}</span><h1>Faça sua inscrição</h1><p>{participant ? `Você está entrando como ${participant.name}. Seus dados de conta serão usados com segurança.` : "Preencha seus dados. Você não precisa criar uma conta."}</p><Field label="Nome completo"><input disabled={loading || Boolean(participant)} onChange={(event) => setName(event.target.value)} placeholder="Seu nome" required value={name} /></Field><Field label="E-mail"><input disabled={loading || Boolean(participant)} onChange={(event) => setEmail(event.target.value)} placeholder="voce@email.com" required type="email" value={email} /></Field><Field label="WhatsApp para contato"><input autoComplete="tel" disabled={loading} inputMode="tel" onChange={(event) => setPhone(event.target.value)} placeholder="(87) 99999-9999" required type="tel" value={phone} /></Field>{error ? <Alert>{error}</Alert> : null}<Button className="button-primary button-block" disabled={loading || event?.isFull} type="submit">{loading ? "Confirmando…" : "Confirmar inscrição"}</Button></form></section></PublicLayout>;
}
...
function ConfirmationPage({ code }: { code: string }) {
  ...
  return <PublicLayout>{error ? <div className="public-error"><h1>Inscrição não encontrada</h1><p>{error}</p><Button className="button-secondary" onClick={() => navigate(paths.eventLookup)}>Tentar outro código</Button></div> : !confirmation ? <p className="loading-text public-loading">Carregando confirmação…</p> : <section className="success-card registration-proof"><div className="success-icon"><AppIcon name="check" size={38} /></div><span className={`badge ${confirmation.status === "ACTIVE" ? "badge-green" : "badge-gray"}`}>{confirmation.status === "ACTIVE" ? "Inscrição ativa" : "Inscrição cancelada"}</span><h1>Comprovante de inscrição</h1><p>Cadastro de <strong>{confirmation.participantName}</strong> em <strong>{confirmation.event.name}</strong>.</p><div className="proof-details"><div><small>Data e horário do evento</small><strong>{dateFormat.format(new Date(confirmation.event.date))}</strong></div><div><small>Local</small><strong>{confirmation.event.location}</strong></div><div><small>Inscrição realizada em</small><strong>{dateFormat.format(new Date(confirmation.createdAt))}</strong></div>{confirmation.checkedInAt ? <div><small>Check-in realizado em</small><strong>{dateFormat.format(new Date(confirmation.checkedInAt))}</strong></div> : null}</div><div className="code-box"><small>Código da inscrição</small><strong>{confirmation.confirmationCode}</strong></div><p className="small-text">Este código comprova o cadastro e poderá ser utilizado no check-in.</p><div className="button-row centered"><Button className="button-secondary" onClick={() => navigator.clipboard?.writeText(confirmation.confirmationCode)}>Copiar código</Button><Button className="button-secondary" onClick={() => window.print()}>Imprimir comprovante</Button><Button className="button-primary" onClick={() => navigate(`/event/${confirmation.event.publicId}`)}>Ver evento</Button></div>{cancellationToken && confirmation.status === "ACTIVE" ? <button className="cancel-link" onClick={() => navigate(`/registration/cancel/${cancellationToken}`)} type="button">Cancelar inscrição</button> : null}</section>}</PublicLayout>;
}
...
function CancellationPage({ token }: { token: string }) {
  ...
  return <PublicLayout>{error ? <div className="public-error"><h1>Não foi possível cancelar</h1><p>{error}</p></div> : !registration ? <p className="loading-text public-loading">Carregando inscrição…</p> : <section className="success-card"><span className="badge badge-blue">Evento360</span><h1>Cancelar inscrição</h1><p>{registration.participantName}, sua inscrição em <strong>{registration.event.name}</strong> está {registration.status === "ACTIVE" ? "ativa" : "cancelada"}.</p>{registration.status === "CANCELLED" ? <Alert kind="success">Esta inscrição já foi cancelada.</Alert> : !confirming ? <Button className="button-danger" onClick={() => setConfirming(true)}>Cancelar minha inscrição</Button> : <div className="confirmation-box"><strong>Tem certeza?</strong><p>Esta ação libera sua vaga.</p><div className="button-row centered"><Button className="button-danger" disabled={loading} onClick={cancel}>{loading ? "Cancelando…" : "Confirmar cancelamento"}</Button><Button className="button-secondary" disabled={loading} onClick={() => setConfirming(false)}>Voltar</Button></div></div>}</section>}</PublicLayout>;
}
